"""CBraMod backbone: patch embedding with spectral features + criss-cross Transformer.

Ported from the original CBraMod codebase.
"""
    ...
    def __init__(self, in_dim: int, out_dim: int, d_model: int, seq_len: int):
        ...
        self.positional_encoding = nn.Sequential(
            nn.Conv2d(in_channels=d_model, out_channels=d_model, kernel_size=(19, 7),
                      stride=(1, 1), padding=(9, 3), groups=d_model),
        )
...
class CBraMod(nn.Module):
    def __init__(self, in_dim: int = 200, out_dim: int = 200, d_model: int = 200,
                 dim_feedforward: int = 800, seq_len: int = 30, n_layer: int = 12,
                 nhead: int = 8):
        super().__init__()
