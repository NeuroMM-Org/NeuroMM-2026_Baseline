"""Task3 EEG + Video late-fusion 5-class trainer."""
...
def detect_video_feature_dim(video_feature_root: str, video_feature_name: str) -> int:
    ...
    return int(arr.shape[0]) if arr.ndim == 1 else int(arr.shape[-1])


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--eeg-model", required=True)
    parser.add_argument("--video-feature-name", required=True)
    ...
    exp_name = (
        f"task3_fusion__{eeg_safe}__{video_safe}__seed{args.seed}__lr{args.learning_rate}"
    )
    model_name_stamp = f"task3_fusion/{eeg_safe}+{video_safe}"
    ...
    logger = get_logger("neuromm26.task3_fusion", str(log_path))
    logger.info(
        "experiment=%s eeg=%s video=%s feat_dim=%d seed=%d",
        exp_name, args.eeg_model, args.video_feature_name, feature_dim, args.seed,
    )

    device = torch.device(args.device)

    train_ds = Task3MultimodalFeatureDataset(
        args.train_val_manifest, "train", args.eeg_feature_root,
        args.video_feature_root, args.video_feature_name,
        target_shape=tuple(args.eeg_shape),
    )
    val_ds = Task3MultimodalFeatureDataset(
        args.train_val_manifest, "val", args.eeg_feature_root,
        args.video_feature_root, args.video_feature_name,
        target_shape=tuple(args.eeg_shape),
    )
    test_ds = None
    ...
    if args.test_manifest and Path(args.test_manifest).exists() and args.test_split:
        test_ds = Task3MultimodalFeatureDataset(
            args.test_manifest, args.test_split, args.eeg_feature_root,
            args.video_feature_root, args.video_feature_name,
            target_shape=tuple(args.eeg_shape),
        )
    ...
    if args.use_class_weights:
        cw = class_balanced_weights(train_ds.eeg_dataset.labels, args.num_classes).to(device)
    ...
    if test_ds is not None and test_loader is not None:
        ...
        logger.info(
            "DONE val_weighted_f1=%.4f@epoch%d  test_weighted_f1=%.4f  test_macro_f1=%.4f  test_acc=%.4f",
